        ...
        if($data)
        {
            ...
            if($checkOtp){
                ...
                if($result)             
                {
                    ...
                    $pat_name = $this->encryptDecrypt("dc",$patientdata->FullName);
                    $type = "changeNumber";
                    $dataArr = array("name"=>$pat_name, "new_mobile_number"=>$NewMobNumber);
                    $sendMail = Utility::callSendMailWithTemplate($pat_email,$type,$dataArr);
                    $response['response_code'] = 1;
                    $response['response_message'] = 'Success';
                }
    ...
    {   
        ...
        if($data)
        {
            ...
            if(isset($ringGroup) && !empty($ringGroup)){
                $getAdv = $this->WebserviceModel->advertiseListing($patientId,$ringGroup); 
            }else{
                $getAdv = $this->WebserviceModel->advertiseListingUniversal($patientId); 
            }
    ...
    function Advertise_search()
    {
        $data = json_decode(file_get_contents('php://input'));
            if($data){
                ...
                $ringGroup = $data->ringGroupId;
                if(isset($ringGroup) && !empty($ringGroup)){
                    $getData = $this->WebserviceModel->Advertise_search($keyword,$ringGroup); 
                }else{
                    $getData = $this->WebserviceModel->Advertise_search_universal($keyword);
                }          
                ...
                if($getData){
                    foreach($getData as $advVal){
                        ...
                        $advVal->InsertDay = "Today";
                            ...
                            $advVal->InsertDay = date('Y-m-d', strtotime($insertdate));
                        }
