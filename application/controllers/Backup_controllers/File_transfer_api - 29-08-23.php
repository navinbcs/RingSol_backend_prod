...
/*******************START: APIs***********************/
    ...
    public function createPdfContentHtmlWithAllergy($content,$dataArray,$userData,$allergy,$pdfInfo,$userAdd){
        ...
        if(isset($userData) && !empty($userData)){
            ...
            $userData->Email = $this->encryptDecrypt("dc",$userData->Email); 
            if($userData->Address){
                $add = str_replace("+",", ",$userData->Address);
            }else{
                $add = "";
            }
            ...
            if(isset($userAdd) && !empty($userAdd)){
                $str .= 'Address - '.$add.', '.$userAdd->City.', '.$userAdd->State.', '.$userAdd->Country.', '.$userData->PinCode.'</td></tr>';
            }else{
                $str .= 'Address - '.$add.', '.$userData->PinCode.'</td></tr>';
            }
        }
