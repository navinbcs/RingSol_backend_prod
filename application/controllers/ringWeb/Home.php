...
class Home extends CI_Controller {
  ...
  public function sendOtpMail()
	{
      $url = "https://apimobile.ring.healthcare:5025/index.php/Webservice/GenerateOtp";
      $headers = array(
    ...
    public function validateOtp(){
        $otp = $_POST['otp'];
        $url = "https://apimobile.ring.healthcare:5025/index.php/Webservice/userOTPValidateForWebPage";
        $headers = array(
				"Content-Type:application/json",
			);
        /*******************************************************/
        ...
        if($res)
        {
          ...
          if(isset($files) && !empty($files)){
            foreach($files as $value){				
				if($value == $userId."_Files_Backup.json"){
					$filesJson = $path.$value;
					$inp = file_get_contents($filesJson);
					$tempArray= json_decode($inp,true);
				}
			}
          ...
          $i = 0;
					foreach($tempArray1 as $tempVal){
            
					 $BackupReportId = $tempVal['BackupReportId'];
					 $fileArr = $this->getdataById($tempArray, $BackupReportId);
					 $tempVal['FileArray'][$i] = $fileArr;
    ...
    function getImgUrlFromBase64(){
      ...
      $url = "https://apimobile.ring.healthcare:5025/index.php/Webservice/generateBase64ToImageLink";
      $headers = array(
				"Content-Type:application/json",
			);
        /*******************************************************/
