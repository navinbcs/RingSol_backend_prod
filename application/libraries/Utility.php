...
class Utility {
	...
	$config['smtp_user'] = 'hello@ring.healthcare';
	$config['smtp_pass'] = 'oW@t7mk2a$7k';
	...
	$ci->email->from('hello@ring.healthcare', 'Identification');
	$ci->email->to('navinbcs@gmail.com');
	$ci->email->subject('Send Email Codeigniter');
	$ci->email->message($massage);
	$ci->email->send();
	echo $ci->email->print_debugger();
		...
		$ci->load->library('email', $config);
        $ci->email
		->from("hello@ring.healthcare", "RING Template")
		// ->to($data->to)
	...
	static function callSendMailWithTemplate_OLD($to,$type,$dataArray){
        $ci = & get_instance();
        
		$ch = curl_init();
			curl_setopt_array($ch, array(
				CURLOPT_URL => 'http://k2key.in/ring/index.php/Mail/sendMailForTesting?' . http_build_query(array(
					'to' => $to,
					'type' => $type,
					'dataArray' => $dataArray,
					'bccArray' => $bccArray
	...
	static function callSendMailWithTemplate($to,$type,$dataArray){
        // $to = "mishraravi520@gmail.com";
        // $type = "loginOtp";
        // $dataArray = array('name'=>'Ravi', 'otp'=>'973467');
        // print_r($data);exit;
        $to = $to;
        $type = $type;
        $dataArray = $dataArray;
		...
		$ci->email->initialize($config);
		$ci->email->set_newline("\r\n");
        if(!class_exists("Mail_template_model")){
            $ci->load->model('Mail_template_model');
        }
        /*********Get Template data*******/

        $bcc ="";
        $emailTemplate = $ci->Mail_template_model->getTemplate($type);
        // print_r($emailTemplate);exit;
        if($emailTemplate){
            if($type == "signUpOtp"){
                $emailMsg = str_replace(
                    array('{name}', '{otp}'),
                    array($dataArray['name'], $dataArray['otp']),
                ...
                $emailMsg = str_replace(
                    array('{name}', '{new_mobile_number}'),
                    array($dataArray['name'], $dataArray['new_mobile_number']),
                    $emailTemplate->Description
                );
                $emailTemplate->subject = $emailTemplate->subject;
            }else if($type == "changeEmailOtp"){
                $emailMsg = str_replace(
                    array('{name}', '{otp}'),
                    array($dataArray['name'], $dataArray['otp']),
                    $emailTemplate->Description
                ...
                $emailMsg = str_replace(
                    array('{name}', '{new_email}'),
                    array($dataArray['name'], $dataArray['new_email']),
                    $emailTemplate->Description
                );
                $emailTemplate->subject = $emailTemplate->subject;
            }else if($type == "convertProfileOtp"){
                $emailMsg = str_replace(
                    array('{name}', '{otp}'),
                    array($dataArray['name'], $dataArray['otp']),
                    $emailTemplate->Description
                ...
                $emailTemplate->subject = $emailTemplate->subject;
            }else if($type == "convertProfile"){
                $emailMsg = str_replace(
                    array('{name}', '{dependent_full_name}'),
                    array($dataArray['name'], $dataArray['dependent_full_name']),
                    $emailTemplate->Description
                );
                $emailTemplate->subject = $emailTemplate->subject;
            }else if($type == "convertProfileFinal"){
                $emailMsg = str_replace(
                    array('{name}', '{ZipLink}'),
                    array($dataArray['name'], $dataArray['ZipLink']),
                    $emailTemplate->Description
                );
                $emailTemplate->subject = $emailTemplate->subject;
            }else if($type == "deleteProfileOtp"){
                $emailMsg = str_replace(
                    array('{name}', '{otp}'),
                    array($dataArray['name'], $dataArray['otp']),
                    $emailTemplate->Description
            ...
            if(true){
                $header = $ci->load->view('mail_template/mailHeader', '', true);
                $footer = $ci->load->view('mail_template/mailFooter', '', true);
                $template = $header.$emailMsg.$footer;
				//$template = $emailMsg;
				$ci->email->from('hello@ring.healthcare', 'RING');
				$ci->email->to($to);
				$ci->email->subject($emailTemplate->Subject);
