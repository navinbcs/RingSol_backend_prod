...
function DuplicateMobileNumber($mobileno){
  ...
  where u.TenantId =".$TenantId);
   ...
   public function checkUserExistance($PatientId,$MobileCode,$MobileNumber,$Otp){
    ...
    {
        $this->db->select("U.UserId DoctorId, U.DisplayName DoctorName, U.PhoneNumber UserPhoneNumber, T.TenantName  TenantName, T.PhoneCode, T.FaxCode, T.PhoneNumber TenantPhoneNuber, T.FaxNumber TenantFaxNumber, T.Address TenantAddress, PSM.SpecialityDescription DoctorSpeciality");
        $this->db->from("UserTenants UT");
        $this->db->join("Tenants T","T.TenantId = UT.TenantId","LEFT");
        $this->db->join("Users U","U.UserId = UT.UserId","LEFT");
    ...
    function advertiseListing($patientId,$ringGroup){
        $this->db->select('*');
    ...
    function advertiseListingUniversal($patientId){
        $this->db->select('*');
		$this->db->from('Advertisements');
        $this->db->where('IsPublished',1);
        $this->db->order_by('InsertDate','DESC');
    ...
    function Advertise_search($keyword,$ringGroup){
        $this->db->select("*");
        $this->db->from('Advertisements');
        $this->db->group_start();
    ...
    function Advertise_search_universal($keyword){
        $this->db->select("*");
        $this->db->from('Advertisements');
        $this->db->group_start();
                $this->db->where("CustomerCompanyName LIKE '%".$keyword."%' ");
